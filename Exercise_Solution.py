...
import datetime
import os
#Read the given csv file and return the list of dictionaries
def readCsv(csvfile):
...
def exercise4(filepath):
    ...
    for i in listvalue:
        if datetime.datetime.strptime(i['StartDate'], '%d-%b-%y') >=startdate and datetime.datetime.strptime(i['StartDate'], '%d-%b-%y')<=enddate:
            datetimeval=datetime.datetime.strptime(i['StartDate'],'%d-%b-%y').strftime('%d/%B/%Y')
            datelist.append(datetimeval)
    return datelist
